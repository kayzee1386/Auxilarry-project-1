###  AUX PROJECT 1: SHELL SCRIPTING

First of all, an instance was launched picking Ubuntu 20.04. Connected to terminal and launch the instance through the public IP

![aws-instance](https://user-images.githubusercontent.com/46185705/132664461-1a7d04e8-302e-46e9-8fb0-b9f92de1b489.jpg)

A directory was created `Shell` and the script file containing the script written are moved into the Shell directory while other files were created as well using
`touch id_rsa.pub id_rsa names.csv`

id_rsa - private key was added to this file

```
-----BEGIN OPENSSH PRIVATE KEY-----

-----END OPENSSH PRIVATE KEY-----
```
wWhile public key was added to this file _ id_rsa.pub

```
ssh-rsa 

```

In the names.csv file, random names were updated as thhe new users

`sudo groupadd developer` was used to add the names in the file `names.csv` together as developers


